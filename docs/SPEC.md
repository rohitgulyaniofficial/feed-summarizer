...
This document describes the current architecture and runtime behavior.
If you find a mismatch between this document and the code, treat the code as authoritative and please update this spec.
...
- `workers/fetcher/`: Fetch feeds with conditional requests and store items
- `workers/summarizer/`: Generate summaries using Azure OpenAI; handles content filters and bisection
- `workers/publisher/`: Publish HTML bulletins and RSS feeds; introduction caching; per-feed scoping; Azure upload
- `models/`: Database access helpers and queue; bulletin/session management
- `workers/scheduler/`: Global and per-feed schedule/interval handling
- `workers/uploader/`: Optional Azure Blob Storage uploader (async `azure-storage-blob` SDK); incremental sync with MD5/mtime checks
- `config.py`: Configuration loading, environment handling, endpoint normalization
- `utils/`: Common helpers (sanitization, atomic writes, hashing)

### Workflow
1. Fetch: `workers/fetcher/` pulls new items from `feeds.yaml` sources using conditional HTTP and stores them in SQLite.
2. Summarize: `workers/summarizer/` builds prompts, calls Azure OpenAI via `services/llm_client.py`, and stores summaries/topics.
3. Publish HTML/RSS: `workers/publisher/` creates session bulletins per group with AI introductions and generates RSS feeds.
4. Passthrough: `workers/publisher/` optionally emits raw passthrough feeds for selected slugs.
5. Upload: `workers/publisher/` can optionally upload generated output via `workers/uploader/` (see `AzureStorageUploader` in `workers/uploader/client.py`).
...
- Calls Azure OpenAI asynchronously via `services/llm_client.chat_completion`.
...
- Bulletin sessions are keyed to the publish timestamp (the time the publisher runs). Large publications can be split into multiple stored bulletin sessions for readability.
...
- Merging is performed in the publishing phase by `workers/publisher/` over the in-memory list of summaries being rendered.
...
- `BM25_MERGE_ENABLED`: enable/disable BM25 fallback (default true).
- `BM25_MERGE_RATIO_THRESHOLD`: minimum mutual ratio in [0,1] (default 0.80).
- `BM25_MERGE_MAX_EXTRA_DISTANCE`: allow BM25 to merge beyond the SimHash threshold by this many bits (default 16).
- `BM25_MERGE_MAX_QUERY_TOKENS`: cap tokens used in BM25 queries (default 8).

Notes:

- BM25 merging is still effectively a no-op unless FTS5 is available and `summary_fts` exists.
- Defaults are defined in `config.py` and may differ from older deployments.
...
For the full, maintained environment-variable reference (including Azure upload, merge tuning, and retention knobs), see `docs/CONFIGURATION.md`.
...
- OPENAI_API_VERSION: API version (taken from env).
- AZURE_STORAGE_ACCOUNT / AZURE_STORAGE_KEY / AZURE_STORAGE_CONTAINER (optional): Azure Blob Storage configuration.
- AZURE_UPLOAD_SYNC_DELETE: if true, delete remote blobs missing locally during upload (use cautiously).
...
- `thresholds.time_window_hours`: recency window used when deciding which entries to ingest and which unsummarized items to consider (default `48`).
...
SQLite with WAL via the `models/` queue. Core tables:
...
See `models/schema.sql` for the full DDL.
...
- Azure Blob Storage Python SDK: [https://pypi.org/project/azure-storage-blob/](https://pypi.org/project/azure-storage-blob/)
